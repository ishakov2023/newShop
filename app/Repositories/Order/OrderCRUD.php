...
class OrderCRUD implements OrderCRUDContract
{
    public function readOrder($id,$userId,$count){
                Product::query()->where(['id'=>$id])->decrement('amount',$count);
                $bask = Basket::query()->where(['product_id'=>$id,'user_id'=>$userId])->get();
    ...
    public function updateOrder($products,$userId){
        ...
        foreach ($products as $product) {
            ...
            foreach ($bask as $basket){
                $order = Order::query()->where('basket_id' ,$basket->id)->get();
                if ($order->isEmpty()){
                        Order::query()->create(['basket_id'=>$basket->id,
                            'user_id'=>$userId]);
